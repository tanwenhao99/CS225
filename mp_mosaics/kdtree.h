...
class KDTree
{
  ...
  private:
    ...
    void quickSort(vector<Point<Dim>>& points, int currDim);
    void buildTree(KDTreeNode*& subroot, vector<Point<Dim>>& points, int currDim);
    KDTreeNode* copy(KDTreeNode* other);
    void destroy();
    Point<Dim> find(KDTreeNode* subroot, const Point<Dim>& query, int currDim) const;
